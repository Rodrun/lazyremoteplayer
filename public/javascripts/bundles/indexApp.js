...
function (_React$Component) {
  ...
  function TextField(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "onChange", function (event) {
      if (_this.props.valueListener) {
        _this.setState({
          value: event.target.value
        }, function () {
          _this.props.valueListener(_this.state.value);
        });
      }
    });
...
function (_React$Component2) {
  _inherits(TextSubmitter, _React$Component2);

  function TextSubmitter(props) {
    var _this2;

    _classCallCheck(this, TextSubmitter);

    _this2 = _possibleConstructorReturn(this, _getPrototypeOf(TextSubmitter).call(this, props));

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this2)), "onClick", function () {
      try {
        // Notify submit callback
        _this2.props.onSubmit(_this2.state.value); // Clear field


        _this2.fieldRef.current.clear();
      } catch (err) {
        console.warn(err);
      }
    });

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this2)), "valueListener", function (newText) {
      _this2.setState({
        value: newText
      });
    });
    ...
    _this2.fieldRef = _react.default.createRef(); // Ref to the TextField

    return _this2;
  }

  _createClass(TextSubmitter, [{
    key: "getButtonName",
    value: function getButtonName() {
      var pName = this.props.buttonName;
